...
Qpv = data.PV.mean(1);
Qmv = 0;
Qav = max(data.AV.mean(1),0);
Qaa = max(data.AA.mean(1),0);
Paa = data.DBP.mean - Raa*(Qav - Qaa); %P_aortic= Raa*(Qav - Qaa)  + Paa

if isnan(data.extra.EDV)
    Vlv = 120; %litterature value
else
    Vlv = data.extra.EDV;
end

if isnan(data.extra.LaESV)
    ...
    Vla = data.extra.LaESV*2; %OBS: Simple estimation
end
